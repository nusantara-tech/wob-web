...
import { getDealHref } from "@/lib/deals";
...
export function DealCard({ deal }: DealCardProps) {
  const dealHref = getDealHref(deal);
  ...
  return (
    ...
    >
      <article
        className="interactive-card group relative aspect-[4/5] min-h-[200px] overflow-hidden rounded-2xl sm:min-h-[220px] md:aspect-[4/3] md:min-h-[230px]"
      >
        <Image
          alt={deal.title}
        ...
        <div className="absolute inset-x-0 bottom-0 p-3 sm:p-4">
          ...
          <div className="mt-2 space-y-1.5 sm:mt-3 sm:space-y-2">
            ...
            <span className="button button--sm button--primary h-7 w-full rounded-lg text-[10px] sm:h-8 sm:rounded-xl sm:text-xs">
              {deal.actionLabel}
            </span>
          </div>
        </div>
