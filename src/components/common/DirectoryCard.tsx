...
import { getDirectoryHref } from "@/lib/directories";
...
export function DirectoryCard({ item }: DirectoryCardProps) {
  const directoryHref = getDirectoryHref(item);
  ...
  return (
    ...
    >
      <Card
        className="theme-card interactive-card group overflow-hidden rounded-2xl border border-slate-200 p-0"
        role="article"
      >
        ...
        <Card.Content className="flex flex-1 flex-col p-3 sm:p-4">
          ...
          <span className="button button--sm button--outline mt-3 h-8 w-full rounded-xl text-xs sm:mt-4">
            Explore
